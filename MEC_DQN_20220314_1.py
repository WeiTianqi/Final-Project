...
输入参数:          Ts： 时隙序号
           ...
           QNet_eval： 当前的评估网络
                 psx:  训练数据的归一化函数类
                 pst:  预测数据的归一化函数类
返回参数：     action: 选择哪个服务器卸载， 0到Num_BS
                   Q： 卸载个数
               Q_now： 根据网络计算的Q值
...
def main():
    ...
    A1=MEC_Qlearning_Unload()
    A1=Smooth(A1,49)
    A2=MEC_Random_Unload()
    A2=Smooth(A2,49)
    A3=MEC_Local_Unload()
    A3=Smooth(A3,49)
    A4=MEC_DQN_Unload()
    A4=Smooth(A4,49)
    fig1 = plt.figure(figsize=(10,10))
    plt.plot(range(0,len(A1)-1), A1[0:len(A1)-1], label='Q-learning',color='r')
    plt.plot(range(0,len(A2)-1), A2[0:len(A2)-1], label='Random',color='g')
    plt.plot(range(0,len(A3)-1), A3[0:len(A3)-1], label='Local',color='b')
    plt.plot(range(0,len(A4)-1), A4[0:len(A4)-1], label='DQN',color='k')
    plt.xlabel('Slots')
    plt.ylabel('Cost')
    plt.title('4种卸载方法对比')
    plt.legend()
    ...
    fig1.savefig(path+'fig1.png',format='png',dpi=100)  
